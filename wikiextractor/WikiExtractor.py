...
def process_dump(input_file, template_file, out_file, file_size, file_compress,
    ...
    reduce_thread = threading.Thread(target=reduce_process, args=(output_queue, output))
    reduce_thread.start()
    ...
    for _ in range(max(1, process_count)):
        extractor_thread = threading.Thread(target=extract_process,
                                            args=(jobs_queue, output_queue, html_safe))
        extractor_thread.daemon = True  # only live while parent process lives
        extractor_thread.start()
        workers.append(extractor_thread)
    ...
    logging.info("Finished %d-thread extraction of %d articles in %.1fs (%.1f art/s)",
